...
export default async function HomePage() {
  ...
  const [
    ...
    supabase
      .from("products")
      .select("id, name, price, sale_price, thumbnail_url, images, is_featured, variants, requires_prescription, stock_quantity, track_stock, condition, product_options(id)")
      .eq("status", "active")
      .eq("is_featured", true)
      .order("created_at", { ascending: false })
      .limit(12),
    ...
    enabledIds.has(CAT_ELECTRONICS) ? carouselSelect().eq("category_id", CAT_ELECTRONICS) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_GROCERY) ? carouselSelect().eq("category_id", CAT_GROCERY) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_FASHION) ? carouselSelect().eq("category_id", CAT_FASHION) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_BEAUTY) ? carouselSelect().eq("category_id", CAT_BEAUTY) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_HOME_KITCHEN) ? carouselSelect().eq("category_id", CAT_HOME_KITCHEN) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_BABY) ? carouselSelect().eq("category_id", CAT_BABY) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_TOYS) ? carouselSelect().eq("category_id", CAT_TOYS) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_HEALTH) ? carouselSelect().eq("category_id", CAT_HEALTH) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_STATIONERY) ? carouselSelect().eq("category_id", CAT_STATIONERY) : Promise.resolve({ data: [] as Row[] }),
    enabledIds.has(CAT_BOOKS) ? carouselSelect().eq("category_id", CAT_BOOKS) : Promise.resolve({ data: [] as Row[] }),
  ]);
  ...
  // Only banners targeted at this emirate (no emirates set = show everywhere)
  const allBanners = (bannersRaw ?? []).filter((b: Row) => {
    const list = b.emirates;
    if (Array.isArray(list) && list.length > 0) {
      return emirate ? list.includes(emirate) : false;
    }
    return true;
  });
  ...
  return (
    <>
      ...
      <HomeHero banners={banners} emirate={emirate} chips={heroChips} />
      ...
      <QuickAccessStrip visible={quickTiles} />

      {/* 5. Category explorer — services only */}
      <CategoryExplorer />

      {/* 6. Electronics carousel */}
      {enabledIds.has(CAT_ELECTRONICS) && (
      <ProductCarousel
        title="Electronics"
        eyebrow="Latest tech"
        emoji="📱"
      ...
      {enabledIds.has(CAT_HOME_KITCHEN) && (
      <ProductCarousel
        title="Home & Kitchen"

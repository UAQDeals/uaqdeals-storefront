...
type AvailCategory = { id: string; name: string; slug: string | null; image_url: string | null; emirates: string[] | null };
  ...
  async (): Promise<{ categories: AvailCategory[]; types: AvailType[] }> => {
    try {
      ...
      const [cats, types] = await Promise.all([
        supabase
          .from("categories")
          .select("id, name, slug, image_url, emirates")
...
export const enabledProductCategories = cache(
  async (): Promise<{ id: string; name: string; slug: string | null; image_url: string | null }[]> => {
    const em = await getEmirate();
    if (!em) return [];
...
// Compact availability bundle for client components (site header):
// enabled category names, enabled vendor-type slugs, and the full set
// of known slugs (so unknown slugs can stay visible, matching the app).
export const availabilitySnapshot = cache(
  async (): Promise<{ cats: string[]; onTypes: string[]; knownTypes: string[] }> => {
    const em = await getEmirate();
    const { categories, types } = await getAvailability();
    const knownTypes = types.map((t) => t.slug);
    if (!em) return { cats: [], onTypes: [], knownTypes };
    return {
      cats: categories.filter((c) => inList(c.emirates, em)).map((c) => c.name),
      onTypes: types.filter((t) => inList(t.emirates, em)).map((t) => t.slug),
      knownTypes,
    };
  }
);

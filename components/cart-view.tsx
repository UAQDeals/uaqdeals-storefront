...
const DELIVERY_THRESHOLD = 50; // AED — free delivery copy only; logic on checkout

export function CartView() {
  const { items, hydrated, setQty, remove, subtotal, totalQty } = useCart();

  // Avoid SSR/CSR mismatch: render a thin skeleton until the store rehydrates.
  if (!hydrated) {
    return (
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center">
        ...
        <p className="mt-2 text-sm text-neutral-600">
          Browse deals or categories to add items.
        </p>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-3">
          {items.map((i) => {
            ...
            const lineOriginal = i.original_price
              ? i.qty * i.original_price
              : null;
            return (
              <div
                key={i.id}
                className="flex gap-3 rounded-2xl border border-[color:var(--brand-border)] bg-white p-3 sm:gap-4 sm:p-4"
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <Link
                        ...
                        className="line-clamp-2 text-sm font-semibold text-neutral-900 hover:text-[color:var(--brand-maroon)]"
                      >
                        {i.name}
                      </Link>
                      {i.variant && (
                    ...
                    <button
                      onClick={() => remove(i.id)}
                      aria-label="Remove"
                      className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-neutral-400 hover:bg-neutral-100 hover:text-red-600"
                    >
                  ...
                  <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
                    ...
                    <div className="text-right">
                      <p className="text-base font-bold text-[color:var(--brand-maroon)]">
                        {aed(lineTotal)}
                      </p>
        ...
        <aside className="h-fit rounded-2xl border border-[color:var(--brand-border)] bg-white p-5 lg:sticky lg:top-20">
          ...
          <dl className="mt-4 space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <dd className="font-semibold">{aed(sub)}</dd>
            </div>
            <div className="flex justify-between">
          ...
          {remainingForFree > 0 && (
            <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
              Add {aed(remainingForFree)} more for free delivery.
            </p>
          )}
          ...
          <Link
            href="/checkout"
            className="bg-brand-gradient mt-5 inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:opacity-95"
          >

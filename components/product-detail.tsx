...
export function ProductDetail({ product: p }: { product: Product }) {
  // Image gallery — dedupe thumbnail + images
  ...
  // Variant selections — default to first option of each group
  ...
  const variantSummary = Object.entries(picked)
    .map(([k, v]) => `${k}: ${v}`)
    .join(", ");
  const lineId =
    p.id + (variantSummary ? "::" + variantSummary.replace(/\s+/g, "_") : "");
    ...
    <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
      ...
      <div className="flex flex-col">
        ...
        <div className="mt-7 flex flex-wrap items-stretch gap-3">
          ...
          {canAdd ? (
            ...
            >
              ...
              Add to Cart · {aed(unitPrice * qty)}
            </button>
          ) : isRx ? (
            <div className="flex flex-1 flex-col items-stretch gap-1">
              <button
                disabled
                className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-neutral-200 px-6 py-3 text-sm font-semibold text-neutral-500"
              >
                <Smartphone className="h-4 w-4" />
                Available in the app
              </button>
              <p className="text-center text-xs text-neutral-500">
                Prescription items can only be ordered through the UAQ Deals
                app.
              </p>
            </div>
          ) : (
        ...
        <p className="mt-8 text-xs text-neutral-500">
          ...
          <Link
            href="/contact"
            className="font-semibold text-[color:var(--brand-maroon)] hover:underline"
          >
            Contact us
          </Link>
        </p>

...
export function StoriesGrid({ tiles, title = "Explore UAQ", eyebrow = "Discover", seeAllHref = "/services", seeAllLabel = "See all" }: {
  tiles: HomeTile[];
  title?: string;
  eyebrow?: string;
  seeAllHref?: string;
        ...
        <div className="mb-6 flex items-end justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h2 className="font-display mt-0.5 text-[24px] font-semibold tracking-tight text-[color:var(--ink)] sm:text-[28px]">{title}</h2>
            </div>
          </div>
              ...
              <Link href={s.href} className="group block">
                <div className="relative w-full overflow-hidden rounded-2xl shadow-[var(--shadow-card)] ring-1 ring-black/5 transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-[var(--shadow-card-hover)]" style={{ aspectRatio: "3/4" }}>
                  {s.imageUrl ? (
                    ...
                    <img src={s.imageUrl} alt={s.title} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-110" />
                  ) : (
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-3.5 text-white">
                    <p className="font-display text-[14px] font-semibold leading-snug drop-shadow">{s.title}</p>
                    {s.titleAr && <p className="mt-0.5 text-[11px] text-white/75">{s.titleAr}</p>}
                  </div>
                </div>
              </Link>
            </Reveal>

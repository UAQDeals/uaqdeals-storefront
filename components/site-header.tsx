...
function DealsMark({ size = 16 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden style={{ flexShrink: 0 }}>
      <defs>
        <linearGradient id={gid} x1="12" y1="22" x2="12" y2="3" gradientUnits="userSpaceOnUse">
...
export function SiteHeader({
  ...
  availability?: { cats: string[]; onTypes: string[]; knownTypes: string[] };
}) {
  // ── Emirate availability gates (everything visible when the
  //    prop is absent, for back-compat) ──────────────────────
  const catOn = (name: string) =>
    !availability || availability.cats.includes(name);
  const typeOn = (slug: string) =>
    !availability ||
    !availability.knownTypes.includes(slug) ||
    availability.onTypes.includes(slug);
  // Product entries gate by their CATEGORY, mirroring the app
  const CAT_GATED: Record<string, string> = {
    grocery: "Grocery",
  ...
  const itemOpen = (slug: string) =>
    CAT_GATED[slug] ? catOn(CAT_GATED[slug]) : typeOn(slug);
  const NAV_CAT_NAME: Record<string, string> = {
    "beauty-fragrance": "Beauty & Fragrance",
    "home-kitchen": "Home & Kitchen",
  };
  const stripOn = (c: { label: string; id: string }) =>
    catOn(NAV_CAT_NAME[c.id] ?? c.label);
  // Same group count/order as SHOP_GROUPS -- only items filtered
  const shopGroups = SHOP_GROUPS.map((g) => ({
    ...g,
    items: g.items.filter((it) => itemOpen(it.slug)),
  }));
                    ...
                    <div className="flex" style={{ minHeight: 320 }}>
                      ...
                      <div className="flex-1 p-4 overflow-y-auto" style={{ maxHeight: 420 }}>
                        ...
                        <div className="grid grid-cols-3 gap-2.5">
                          {shopGroups[activeGroup].items.map((item) => {
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100" style={{ background: "linear-gradient(to right, #C72931, #8E1B3A)" }}>
          {mobileGroup !== null ? (
            <button onClick={() => setMobileGroup(null)} className="flex items-center gap-2 text-[13.5px] font-semibold text-neutral-700">
              <ChevronRight className="w-4 h-4 rotate-180" />{label(shopGroups[mobileGroup])}
            </button>
          ) : (
        ...
        <div className="flex-1 overflow-y-auto">
          {mobileGroup === null ? (
            ...
            <div className="py-2 px-2">
              <p className="px-3 py-2 text-[10.5px] font-bold tracking-[2px] uppercase text-neutral-400">{label(shopGroups[mobileGroup])}</p>
              {shopGroups[mobileGroup].items.map((item) => (

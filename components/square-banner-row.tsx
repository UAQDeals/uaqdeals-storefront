...
export function SquareBannerRow({ section, isAr }: { section: SquareSection; isAr: boolean }) {
  ...
  const hasBg = section.bgType && section.bgType !== "none";
  const bgStyle: CSSProperties = {};
  if (section.bgType === "color" && section.bgColor) {
    bgStyle.background = section.bgColor;
  } else if (section.bgType === "gradient") {
    bgStyle.background = `linear-gradient(135deg, ${section.bgColor || "#f3e8ff"}, ${section.bgColor2 || "#e0f2fe"})`;
  } else if (section.bgType === "image" && section.bgImageUrl) {
    bgStyle.backgroundImage = `url(${section.bgImageUrl})`;
    bgStyle.backgroundSize = "cover";
    bgStyle.backgroundPosition = "center";
  }
  const light = section.textLight;
  const titleCls = light ? "text-white" : "text-[color:var(--ink)]";
  const eyebrowCls = light
    ? "text-[11px] font-bold uppercase tracking-[0.2em] text-[color:var(--brand-gold)]"
    : "eyebrow";
  ...
  return (
    <section className="border-t border-[color:var(--brand-border)] py-12 md:py-14">
      <div className="mx-auto max-w-[1320px] px-5 md:px-8">
        <div
          className={hasBg ? "relative overflow-hidden rounded-3xl p-5 shadow-[var(--shadow-card)] md:p-7" : ""}
          style={hasBg ? bgStyle : undefined}
        >
          {section.bgType === "image" && <span className="absolute inset-0 bg-black/25" aria-hidden />}
          {(title || subtitle) && (
          ...
          <div className="relative flex gap-4 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">

...
export function DealsStrip({ deals }: { deals: DealCard[] }) {
      ...
      <div className="mb-4 flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Fresh picks at unbeatable prices.
          </p>
        </div>
        ...
        {deals.map((d) => {
          ...
          return (
            <Link
              key={d.id}
              href={`/deals/${d.id}`}
              className="group w-[180px] shrink-0 snap-start overflow-hidden rounded-2xl border border-[color:var(--brand-border)] bg-white sm:w-[210px]"
            >

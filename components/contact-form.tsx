...
export function ContactForm() {
  ...
  if (sent) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-[color:var(--brand-border)] bg-white p-10 text-center">
        ...
        <p className="mt-1 text-sm text-neutral-600">{t("sentDesc")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-[color:var(--brand-border)] bg-white p-5 sm:p-6">
      ...
      <button onClick={submit} disabled={sending} className="bg-brand-gradient mt-5 inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white shadow-sm disabled:opacity-60">
